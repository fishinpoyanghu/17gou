...
class FinanceData extends BaseData {
    ...
    public function getFinance($start,$end){
        ...
        $sql = "select from_unixtime(`ut`,'%Y-%m-%d') as t,abs(sum(money)) as total from {$db_cfg['tbl']} where `desc`='消费' {$and} and `uid` not in (select `uid` from 17gou.t_false) group by t";
		...
		$db_cfg = load_db_cfg($this->cfg_name, $this->tbl_alias, '', 'r');
		$db_op = DbOp::getInstance($db_cfg);
		$and = " and a.`ut` >= {$start} and a.`ut` <= {$end}";
		if($keyword){
		...
		$sql = "select a.*,c.`title`,d.`nick`,group_concat(a.`activity_num`) as num_str from {$table} where 1 {$and} group by a.`order_num`,a.`activity_id` order by a.`ut` desc {$limit}";
		$count = $db_op->queryRow("select count(DISTINCT a.`order_num`,a.`activity_id`) as total from {$table} where 1 {$and}");
		if(!$count['total']) return array();
		$total = $db_op->queryRow("select count(*) as total from {$table} where 1 {$and}");
		return array(
