...
return array(
		...
		'base' => array(
				...
				'db_tbl_alias' => 'img',
				'pkid' => 'img_id',
				'autoidCode' => C('AUTOID_IMG'),
				'title' => '编辑器图片管理',
				'where' => array(
						'dt' => 0,
				),
				'orderby' => 'ORDER BY img_id DESC',
				'pagesize' => 10, // 列表的分页数目
				...
				'concatList' => array(
						array(
								'db_cfg_name'  => 'core',
								'db_tbl_alias' => 'img_category',
								// 主表的字段xxx(由下面concatid定义)外键关联到目录表的主键字段yyy(由下面pkid定义)
								'pkid' => 'img_category_id',
								'concatid' => 'img_category_id',
...
// 										'pid'  => 'group_pid',
				...
				'search' => array(
						...
						'searchCid'   => 'img_category_id', // 定义需要检索的目录ID，如果未定义，则不进行目录检索
		...
		'classify' => array(
				'multiBtnName' => '修改分类',
				'name' => '分类', // 分类的字段描述
				'categoryId' => 'img_category_id', // 定义存储分类的字段
				'require' => true,
				'defaultSet' => array(
						'ut'  => 'time', // 调用time()函数
				),
