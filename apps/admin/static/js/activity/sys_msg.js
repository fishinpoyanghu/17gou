define(function(require, exports, module) {
	...
	$('#search').on('click',function(){
		...
		if(key){
			location.href = '?c=activity&a=sysMsg&keyword='+key;
		}
	});
	...
	$('.js-sub').on('click',function(){
		...
		$.getJSON("?c=activity&a=saveSysMsg",{"data":data,"id":id},function(re){
			if(re.code > 0){
	...
	function sysMsgDel(id){
		dialog({
			...
			ok: function() {
				this.close().remove();
				$.getJSON("?c=activity&a=sysMsgDel",{"id":id},function(re){
					if(re.code > 0){
						location.reload();
					}else{
						altDialog('删除失败');
