...
define(function(require, exports, module) {
  ...
  var Imglib = function(opts, callback) {
    this.popType = {
      userPic: true,  // 有我的图片
      newPic: true,   // 有新图片
  ...
  Imglib.prototype.init = function(opts) {
    this.initElements(opts);
    this.initTemplate();
    this.usePic();
  };

  Imglib.prototype.initElements = function(opts) {
    ...
    var tplNewPic = '<div class="tab-pane" id="tabpane_newpic">' +
                      ...
                      '</div>' +
                    '</div>';
    this.modalHtml = '<div class="modal-imglib dp-modal modal fade" id="modal_imglib">' +
  ...
  // 初始化弹窗
  Imglib.prototype.initTemplate = function() {
    var self = this;
    var render = template.render(self.modalHtml);
    var modal = render(self.popType);
    var $modal;

    if($('#modal_imglib').length < 1) {
        $('body').append(modal);
    }
    $modal = self.$modal = $('#modal_imglib');
  ...
  // 上传新图片
  Imglib.prototype.uploadNewPic = function () {
        var self = this;
        ...
        var uploador = new Uploador({
            ...
            submitUrl: '?c=pic&a=get_url&size=' + self.popType.size
        });
  ...
  // 初始化我的图片
  Imglib.prototype.initUserPic = function() {
    var self = this;
    var $modal = self.$modal;
  ...
  // Ajax更新数据
  Imglib.prototype.updataPic = function(postData, target) {
    var self = this;
...
//        alert(re.msg);
  ...
  // 绑定分页按钮点击事件
  Imglib.prototype.bindPageEvent = function (dom, postData, target) {
    var self = this;
  ...
  // 确定使用图片
  Imglib.prototype.usePic = function() {
    var self = this;
    var $modal = self.$modal;
  ...
  Imglib.prototype._callback = function(url) {
      // 得到url执行回调函数
      if (this.callback) {
          this.callback(url);
      }
