define(function(require, exports, module) {
	...
	$('.js-add-link').on('click',function(){
		...
		$.getJSON("?c=goods&a=addLink",{"id":window.BANNER_ID,"goods_id":goods_id,"url":encodeURIComponent(url),"type":type},function(re){
			$('#add_link').modal('hide');
			if(re.code > 0){
	...
	$('.js-use').on('click',function(){
		...
		dialog({
			...
			ok: function() {
				this.close().remove();
				$.getJSON('?c=goods&a=editBanner',{'id':id},function(re){
					if(re.code > 0){
						location.reload();
					}else{
						dialog({
	...
	$('.js-del').on('click',function(){
		...
		dialog({
			...
			ok: function() {
				this.close().remove();
				$.getJSON('?c=goods&a=delBanner',{'id':id},function(re){
					if(re.code > 0){
						location.reload();
					}else{
						altDialog('删除失败！');
	...
	$('.js-add').on('click',function(){
		var piclib = new Piclib({
			...
			iconLib: '',
			size:'750*445',
			tipsnew:'上传的图片小于1MB，尺寸为750*445，格式为jpg，png。'
		}, function(url) {
			$.getJSON('?c=goods&a=addBanner',{'url':url},function(re){
				if(re.code > 0){
					 location.reload();
				}else{
					altDialog('添加失败！');
	...
	$('input[name=sort]').on('change',function(){
		...
		$.getJSON('?c=goods&a=sortBanner',{'id':id,'sort':sort},function(re){
			if(re.code > 0){
				altDialog(re.msg,function(){
					location.reload();
				});
