define(function(require, exports, module) {
    ...
    $('.remen').click(function(){
        ...
        $.getJSON('?c=goods&a=remen&id='+id,function(re){
    ...
    $('.tianjia').click(function(){
        var id = $(this).attr('data-id');
        $.post('?c=goods&a=shouin',{"id":id},function(re){
            if(re.code > 0){
                altDialog(re.msg,function(){
                    location.href = "?c=goods&a=classify";
                });
            }else{
                altDialog(re.msg);
            }
    ...
    $('.qudiao').click(function(){
        var id = $(this).attr('data-id');
        $.post('?c=goods&a=shouout',{"id":id},function(re){
            if(re.code > 0){
                altDialog(re.msg,function(){
                    location.href = "?c=goods&a=classify";
                });
	...
	function delGoodsCfy(id){
		var ids = id.join(',');
		$.getJSON('?c=goods&a=delGoodsCfy',{'ids':ids},function(re){
			if(re.code > 0){
				location.reload();
			}else{
				errorDialog('删除失败！');
	...
	$('.js-sub').on('click',function(e){
		...
		$.getJSON('?c=goods&a=editGoodsCfy',{'id':id,'name':name,'url':url,'pid':pid},function(re){
			if(re.code > 0){
				errorDialog(re.msg,function(){
					location.href = "?c=goods&a=classify";
				});
	...
	$('.js-modify').on('click',function(){
		...
		$.post('?c=goods&a=modifyGoods',{"ids":id,"type":type},function(re){
			$('#m_modify').modal('hide');
			if(re.code > 0){
				errorDialog(re.msg,function(){
					location.reload();
	...
	$('.js-goods-del').on('click',function(){
		...
		dialog({
			...
			ok: function() {
				this.close().remove();
				$.getJSON('?c=goods&a=delGoods',{"id":id},function(re){
					if(re.code > 0){
	...
	$('.js-goods-start').on('click',function(){
		...
		dialog({
			...
			ok: function() {
				this.close().remove();
				$.getJSON('?c=goods&a=startFirst',{"id":id},function(re){
					if(re.code > 0){
	...
	$('.js-goods-stop').on('click',function(){
		...
		dialog({
			...
			ok: function() {
				this.close().remove();
				$.getJSON('?c=goods&a=stop',{"id":id},function(re){
					if(re.code > 0){
						errorDialog('操作成功！',function(){
							location.reload();
						});
