...
class NcListMod extends BaseMod{
	 ...
	 * @return multitype: 2017/2/20 where 数组已修改成跟 getDataJoinTable 一致
	 */
				...
				if (is_array($v)) {
					...
					if (($v[1] == 'in') && is_array($v[0])) {
						...
						$ret[] = $k.' '.$v[1].' ('.implode(',', $v[0]).')';
						  ...
						  if($k=='_complexsql'){  //添加复合查询语句  demo nc_activity-->getActivityList
						  	 foreach($v['whereArr'] as $_key =>$_val){
						  	 	$_complexsql.=' '.$_val[0].' '.$_val[1].' \''.$_val[2].'\''.' '.$v['_logic']; 
						  	 }
						  	 $ret[]=' ( ' .rtrim($_complexsql,$v['_logic']).' ) ';
	 ...
	 *获取连表数据总页数
	 */
	public function getJoincount($join,$where = array(), $column = '',$noEmpty = true){
		if(empty($this->dbConf) ) return array(); 
		$whereSql = ''; 
		$selectSql = '*';
		//组装where
		...
		$db_op = DbOp::getInstance($this->dbConf);
		$sql = "select count($column) as count from {$join['from']} ";
		//组装join
		if(isset($join['join'])){
			foreach($join['join'] as $val){
				$sql .= " {$val['join_type']} {$val['join_table']} on ".implode(" and ", $val['on']);
