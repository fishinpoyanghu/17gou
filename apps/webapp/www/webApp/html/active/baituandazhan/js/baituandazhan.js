...
define(
	...
	function(app) {
		...
		app.controller('baituandazhanCtrl', baituandazhanCtrl);
		baituandazhanCtrl.$inject = ['$scope', '$state', '$stateParams', 'GoodsModel', 'PintuanModel', 'ActivityModel', 'MyUrl', 'ToastUtils', 'userInfo', 'Global', 'weChatJs', '$ionicHistory', '$timeout', '$ionicSlideBoxDelegate','$ionicScrollDelegate'];

		function baituandazhanCtrl($scope, $state, $stateParams, GoodsModel, PintuanModel, ActivityModel, MyUrl, ToastUtils, userInfo, Global, weChatJs, $ionicHistory, $timeout, $ionicSlideBoxDelegate,$ionicScrollDelegate) {
			(function init() {
			...
			function refresh() {
				...
				PintuanModel.baituan_homepage('', onSuccess, onFailed, onFinal);
			...
			function getHomeNewPublish() {
				PintuanModel.getbaituanNewPublish(function(xhr, re) {
					var code = re.code;
					if(code == 0) {
						var data = re.data;
			...
			$scope.timeoutCallback = function(activity) {
				PintuanModel.baituan_getDetail_info(activity.activity_id, function(response, data) {
					if(data.code == 0) {
