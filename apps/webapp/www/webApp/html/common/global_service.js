...
define([
  ...
  'use strict';
  app
    .factory('Global',['$rootScope','$parse','trolleyInfo','Storage',function($rootScope,$parse,trolleyInfo,Storage){
      ...
      function setNoticeNew(msg){
        ...
        //$rootScope.all_new = new Date().getSeconds();
        //setDomTips();
          zjx();
      }

      function setDomTips(){
        var dom = document.getElementById('tab_my');
        domController.setAttr(dom,'badge',10);
        var model = $parse(dom.getAttribute('badge'));
        //ssjjLog.log(model($rootScope));
        model.assign($rootScope,10);
      }
      ...
      $rootScope.trolleySum = function(){
        return trolleyInfo.getGoodsInfo().length == 0 ? '' : trolleyInfo.getGoodsInfo().length;
      };
        function zjx(){
            if($rootScope.trolleySum()>0){
                $rootScope.all_new= $rootScope.all_new+$rootScope.trolleySum();
            }
        }
        zjx();
      var browser={
      ...
      function isInAPP(){
        return ionic.Platform.isWebView();//navigator.camera&&browser.versions.mobile
      }

      function getInviteCode(){
        return Storage.get('invidecode');;
      }

      function setInviteCode(invidecode){
        // inviteCode = invidecode;
        Storage.set('invidecode',invidecode);
      }
      function removeInviteCode(invidecode){
        Storage.remove('invidecode');
      }
      return{
        isInAPP : isInAPP,
        ...
        removeInviteCode:removeInviteCode,
