...
define(
    ...
    function(app,funParabola){
        'use strict';
        app
            .controller('new_goodsController',['$scope','$stateParams','$timeout','ToastUtils','ClassificationService', 'ActivityModel','PintuanModel','trolleyInfo','$window','Global','$ionicScrollDelegate','$state','$ionicPopup','$ionicGesture','Storage',
                function($scope,$stateParams,$timeout,ToastUtils,ClassificationService,ActivityModel,PintuanModel,trolleyInfo,$window,Global,$ionicScrollDelegate,$state,$ionicPopup,$ionicGesture,Storage){
                    ...
                    $scope.getData = function(doRefresh) {
                        ...
                        PintuanModel.pintuan_homepage($scope.goods_type_id,null,order_key,'desc',($scope.page-1) * 20 + 1,20,'0','1', function(xhr, re) {
                            var code = re.code;
                            if (code == 0) {
                                var data = re.data;
                                var len = data.length;
                    ...
                    $scope.fetchData = function(goods_type_id) {
                        if(!$scope.isLoadFinished) return;
                        $scope.activity_type=1;//1.普通拼团 2.幸运拼团 3.团长免费
                        // $scope.order_type = '';
