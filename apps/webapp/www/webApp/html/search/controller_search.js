...
define(
	...
	function(app) {
		'use strict';
		app
			.controller('SearchController', ['$scope', '$state', '$stateParams', '$location', 'AppModel', 'ToastUtils', function($scope, $state, $stateParams, $location, AppModel, ToastUtils) {
				$scope.searchContent = '';
				...
				$scope.num = 0;
				$scope.numb;
				$scope.hotWords = ['牙膏', '手机', '东方名剪', '英雄联盟皮肤', '衬衫', '牛仔裤', 's','东方名剪', '英雄联盟皮肤', '衬衫', '牛仔裤', '东方名剪', '英雄联盟皮肤'];
				//清除输入的文本
...
//					$state.go("tab.mainpage")
				...
				$scope.goToResult = function(keyword) {
					...
					$scope.textarea.unshift(keyword.slice(0,7));
//					copy_newtextarea();
//					at_first();
				...
				$scope.$on('$ionicView.beforeEnter', function(ev, data) {
					AppModel.getSearchHotWord($stateParams.productType, function onSuccess(response, data) {
						if(data.code == 0) {
