...
class Log {
    ...
    static function record($message,$level=self::ERR,$record=false) {
        ...
        if($record || in_array($level, explode(',', C('LOG_RECORD_LEVEL')))) {
    ...
    static function save($type=self::FILE, $destination='', $extra='') {
    	...
    	if(empty($destination))
            $destination = get_app_root().'/cache/'.date('y_m_d').'.log';
        
        if(self::FILE == $type) { // 文件方式记录日志信息
            //检测日志文件大小，超过配置大小则备份日志文件重新生成
            if(is_file($destination) && floor(C('LOG_RECORD_SIZE')) <= filesize($destination) )
