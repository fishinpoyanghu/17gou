...
class ThumbHandler
{
    ...
    var $img_display_quality = 90;// 图片显示质量,默认为75
    ...
    var $corner_radius=5; // 半径
    var $corner_angle=0; // 角度
    var $corner_img=''; // 图片圆角png透明文件
    var $corner_top_left=true; // 左上是否圆角
    var $corner_bottom_left=true; // 左下是否圆角
    var $corner_bottom_right=true; // 右上是否圆角
    var $corner_top_right=true; // 右上是否圆角

    var $cut_type=0; // 剪切类型
    var $cut_anyway=0; // 标记当宽和高都小于置顶剪切大小时，要不要裁剪
     ...
     * 设置圆角图片，必须是png
     *
     * @param string img, 圆角png图片
     * @param int radius, 半径, default:5
     * @param int angle, 角度, default:0
     * @param boolean, top_left, 左上是否圆角, default:true
     * @param boolean, bottom_left, 左下是否圆角, default:true
     * @param boolean, bottom_right, 右下是否圆角, default:true
     * @param boolean, top_right, 右上是否圆角, default:true
     */
    function setCornerImg($img, $radius=5, $angle=0, $top_left=true, $bottom_left=true, $bottom_right=true, $top_right=true) {
    	
    	$this->corner_img = $img;
    	$this->corner_radius = $radius;
    	$this->corner_angle = $angle;
    	$this->corner_top_left = $top_left;
    	$this->corner_bottom_left = $bottom_left;
    	$this->corner_bottom_right = $bottom_right;
    	$this->corner_top_right = $top_right;
    }
    ...
    /**
     * 设置当宽和高都小于置顶剪切大小时，要不要裁剪
     * 
     * @param int anyway, 0/1
     */
    function setCutAnyway($anyway) {
    	$this->cut_anyway=$anyway;
    } 
...
//        $this->_drawRoundedCorner();
     ...
     * @throws SimpleException
     +----------------------------------------------------------
     */
    function cut($image, $type='', $filename='', $x=0, $y=0, $w=20, $h=20, $interlace=true) {
        ...
        if($info !== false) {
            ...
            $type =  $info['type']; // $pathinfo['extension'];dump($type);exit;
            ...
            if(function_exists("ImageCopyResampled"))
                ...
                imagecopyresized($thumbImg, $srcImg, 0, 0, $x, $y, $w, $h,  $w,$h); 
     ...
     * 画圆角
     */
    function _drawRoundedCorner() {
    	
    	if (empty($this->corner_img)) return;
    	
    	if (!is_file($this->corner_img)) {
    		die('corner_img is not exist error!');
    	}
    	
    	$corner_source = imagecreatefrompng($this->corner_img);
    	...
    	$img_type = $this->_getImgType($this->corner_img);
    	
    	$white = ImageColorAllocate($image,255,255,255);
    	$black = ImageColorAllocate($image,0,0,0);
    	
    	// Top-left corner
		if ($this->corner_top_left) {
		    $dest_x = 0;  
		    $dest_y = 0;  
		    imagecolortransparent($corner_resized, $black); 
		    imagecopymerge($this->h_dst, $corner_resized, $dest_x, $dest_y, 0, 0, $corner_width, $corner_height, 100);
		} 
		
		// Bottom-left corner
		if ($this->corner_bottom_left == true) {
		    $dest_x = 0;  
		    $dest_y = $this->dst_h - $corner_height; 
		    $rotated = imagerotate($corner_resized, 90, 0);
		    imagecolortransparent($rotated, $black); 
		    imagecopymerge($this->h_dst, $rotated, $dest_x, $dest_y, 0, 0, $corner_width, $corner_height, 100);  
		}
		
		// Bottom-right corner
		if ($this->corner_bottom_right == true) {
		    $dest_x = $this->dst_w - $corner_width;  
		    $dest_y = $this->dst_h - $corner_height;  
     ...
     */
    function mvGif($src_path, $dst_path) {
    	
    	file_put_contents($dst_path, file_get_contents($src_path));
    }
    ...
    function _output()
    {
        ...
        if(function_exists($func_name))
        {
            ...
            if (($func_name == 'imagepng') && ($this->img_display_quality > 9)) {
            	$this->img_display_quality = intval($this->img_display_quality/10);
            	if ($this->img_display_quality > 9) $this->img_display_quality = 9;
            }
    ...
    function _setNewImgSize($img_w, $img_h=null)
    {
        ...
        if(2 == $num)
        {
            ...
            switch($this->cut_type)
            {
                ...
                case 1:
                    // 如果图片是缩小剪切才进行操作
                    if($this->cut_anyway || ($rate_w >= 1 || $rate_h >=1))
                    {
