      ...
      function addCartItem($event,goodsItem,addNum,home) {  
        if(home=='home'){ //表示首页直接跳转到支付页面  
          var shopData=[{ //拼接json以符合数组参数
            ...
            join_number:goodsItem.activity_type==2?goodsItem.join_number*10:
            			goodsItem.activity_type==4?goodsItem.join_number*(goodsItem.need_num/2):
            			goodsItem.join_number,
            isSelected:true
          }]   
...
//            console.log(root.p_cartInfo.$model)
              avalon.cookie.set('cartInfo',JSON.stringify(root.p_cartInfo.$model),{setmaxage:-1})
              avalon.Storage.set('cartInfo',root.p_cartInfo.$model)
            }
          })
      ...
      function addItem(goodsItem,addNum) { 
          ...
          if (goodsIndex != null) {
              ...
              }else if(goodsItem.activity_type == 4){
              		if (goodsItem.join_number*(goodsItem.need_num/2) + number <= goodsItem.need_num) {
              			number = goodsItem.join_number*(goodsItem.need_num/2) + number;
              		}else{
              			number = goodsItem.need_num/2;
              		}
              } else {
              ...
              if (goodsItem.activity_type == 4) {
              	goodsItem.join_number = goodsItem.join_number * (goodsItem.need_num/2);
              }
      ...
      .state("twoPersons", {
        ...
        views: {
          "": {
            templateUrl: "html/twoPersons.html", 
            controllerUrl: "page/twoPersons/twoPersons.js" 
          }
        }
      })
      .state("announced", {
