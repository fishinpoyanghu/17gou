...
//              indexVM.navItem = [].concat(classifyList);
                ...
                var newClassify = allClassify.slice(0);
                indexVM.classifyList = newClassify.reverse();
//              indexVM.classifyList = newClassify;
...
//                   indexVM.navItem.push(classifyList[indexVM.blockIndex].name); 
                    ...
                    if (data[0].activity_type == 2) {
                    	var tenYuan = $($('.yConCenter')[$('.yConCenter').length-1]);
                    	var $a = tenYuan.children().children().filter('a');
                    	$.each($a,function(i,n){
                    		$(n).attr('href','#!/tenyuan');
                    	})
//                  	console.log(tenYuan.children().children().filter('a'))
                    }
                } else {
                    layer.msg(re.msg)
                }
